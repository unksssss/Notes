...
## Blueprint 坐标系（Day 12/14 连续失分点 ⚠️）

**核心公式：局部坐标 × Transform 矩阵 = 世界坐标**

- Blueprint 是**可复用资产**（一条蓝图可生成多条绳索实例），所以控制点存 **Actor 局部坐标**，运行时 Obi 通过 Actor 的 Transform 变换到世界空间
- **Actor Transform 放哪，绳索就从哪生成**——它有决定性影响！
