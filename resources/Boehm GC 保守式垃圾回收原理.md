...
## 分代 GC 详解：为什么快 + 晋升条件（Day 31 教学）

### 出发点：弱分代假说（Weak Generational Hypothesis）

绝大多数对象都是**"朝生夕死"**的——出生后很快不再被引用（临时字符串、每帧中间对象）。若 GC 每次全堆扫描，99% 精力花在"检查马上要死的对象"，老对象被反复检查却从没死过 = 纯浪费。

```
对象生命周期分布：约 95% 短命（毫秒~秒级），约 5% 长寿（全程存活）
```
...
| 代 | 外号 | 住什么对象 |
|---|---|---|
| **Gen0** | 新生儿房 | 刚 new 出来的对象 |
| **Gen1** | 青年房 | 经历过 1 次 GC 还活着的 |
| **Gen2** | 元老院 | 经历过 2 次 GC 还活着的 |
| **LOH**（大对象堆） | 特等房 | >85KB 大对象直接进，不参与分代搬家 |

### 为什么分代就快（两个大招）

1. **回收只扫"新生儿房"**：多数 GC 只对 Gen0 回收——扫描范围小、速度快；Gen1/Gen2 平时不被检查，空间不足才逐级"惊动"。成本集中在"死亡高发区"。
2. **新生代分配用指针碰撞（bump pointer）**：Gen0 连续整块，分配 = 指针后移，O(1)。不用遍历空闲链表找坑。

### 晋升条件（重点）

```
Gen0 满 → 触发 GC（只扫 Gen0）
   ├─ 没被引用 → 回收 🗑️
   └─ 幸存 → 晋升 Gen1（+1 岁）
Gen1 满 → GC（扫 Gen1 + Gen0）→ 幸存晋升 Gen2
Gen2 满 → Full GC（全堆扫描，STW 最久）⚠️
```

**一句话：在 GC 中幸存 = 晋升一代**。被 `static` 字段/单例/全局引用吊着的对象一路升到 Gen2，从此很少被检查——代价是 **Gen2 里的泄漏几乎查不出来**（内存泄漏最难抓的原因）。

### 类比 🏢：裁员审查

- Gen0 = 试用期员工（流动最大，每轮裁员先查他们）
- Gen1 = 转正员工（偶尔查）
- Gen2 = 元老（默认不查，除非大危机 = Full GC）

### 对 Unity 开发的启示

- 每帧 new 小对象 → Gen0 堆积 → 频繁 GC → 卡顿
- **对象池** = 让对象"永远不死"（复用），绕开 Gen0 死亡高发区 → 配合峰值控制（见 [[对象池实现]]）
- 别把一次性对象误存 static → 它升到 Gen2 赖着不走 → 泄漏
